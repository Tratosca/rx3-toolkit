...
Rasterisation was matched rather than the file. Pioneer's glyphs quantise to sixteen grey levels, which is 4-bit anti-aliasing, and their vertical stems are solid with the anti-aliasing only on curves — hinting. Supersampling matched the pixel totals but softened the stems and looked wrong at zoom, so the lettering is drawn once at final size, hinted, to reach their stroke mass, with the coverage quantised to sixteen steps.
...
- Reinserting a drive no longer restarts the player. A drive pulled out as `sda` comes back as `sdb`, so it mounts somewhere else; the sidecar directory is read once at load time, so a moved path meant stopping and relaunching — which froze the screen and emptied the media list, which is what made the drive get pulled again. The player is handed one fixed path, re-pointed on each insertion, and a reinsertion now changes nothing.
