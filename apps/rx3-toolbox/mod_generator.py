...
class ModGeneratorPane(ttk.Frame):
    ...
    def _build_interface(self) -> None:
        ...
        ttk.Label(container, text="Build an RX3 USB runtime", style="Title.TLabel").grid(
        ...
        theme.path_row(container, 7, "Output folder or USB drive", self.output_path, self._choose_output)
    ...
    def _load_patches(self) -> None:
        ...
        patches = [
            patch
            for patch in discover_patches(self.root_resources, self.firmware.get())
            if patch.selectable
        ]
        for row, patch in enumerate(patches):
            ...
            theme.wrapping(ttk.Label(
                self.patch_frame, text=patch.description, style="Muted.TLabel",
