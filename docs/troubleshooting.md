...
Applying a module restarts the player. The restart misses the announcement the drive made when you inserted it, so the drive is still there but no longer listed. Since `0.5.1` the mod has that announcement repeated once the restarted player is up, about a second later. In the log it reads `re-announced /dev/... to the hotplug handler`.

That is the runtime side of it. The drive can still take several seconds to come back, which is being looked into — see **Known issues** in the changelog. Waiting is the only thing to do; the drive is not lost.
